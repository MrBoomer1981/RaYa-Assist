...
### 2026-04-22 — Утренний дайджест: актуальные новости за сутки

**Проблема:** дайджест использовал обычный `search()` вместо специализированного `news_search()`, брал только 2 результата на тему, темы выбирались рандомно и без даты в запросе — поисковик мог возвращать устаревшие материалы.

**Изменён файл: `app/agents/morning_agent.py`**

- `_NEWS_QUERIES` (рандомный пул из 10) → `_NEWS_TOPICS` (фиксированные 5 тем: AI, Мир, Бизнес, Наука, Психология) — каждый день все 5, без пропусков
- `svc.search()` → `svc.news_search()` — Tavily с `topic="news"` приоритизирует свежие новостные публикации, DDG как fallback
- Запросы теперь содержат текущую дату (`22 April 2026`) — поисковик ранжирует материалы за последние сутки выше архивных
- `max_results` увеличен с 2 до 4 на тему, снипет с 800 до 1200 символов — больше деталей для пересказа
- LLM-промпт переписан: требует конкретики (имена, цифры, компании), 2-3 предложения на тему вместо 1-2, темы без новостей пропускаются
- Заголовок дайджеста: `📰 Новости за последние сутки` вместо `📰 Дайджест дня`

**Оптимизация для 25+ пользователей:** `news_search` кэшируется в `_cache_news` (TTL из SearchService) — при совпадении запросов между пользователями повторный поиск не делается.

---

### 2026-04-22 — Роутер: трёхуровневая классификация

**Проблема:** простые разговорные сообщения ("супер", "понял тебя") и вопросы-факты ("сколько времени в лондоне") попадали в LLM роутер, который ошибочно отправлял их в `text`-агент. Описание агента "работает с текстом" провоцировало LLM считать любое текстовое сообщение подходящим.

**Изменены файлы: `app/agents/router.py`, `app/agents/registry.py`**

**router.py — Уровень 0 (до ключевых слов и LLM):**
- `_CONVERSATIONAL_RE` — regex для коротких разговорных реплик: "ок", "супер", "понял", "спасибо", "привет" и ~25 вариантов → немедленно `raya`, уверенность 1.0, без LLM
- `_RAYA_RE` — regex для явных raya-запросов: время, погода, курсы, новости, факты → `raya` без LLM если сообщение ≤12 слов
- Итого три уровня: **разговор → ключевые слова → LLM**

**router.py — Промпт LLM роутера:**
- Добавлены конкретные примеры для пограничных случаев (5 пар сообщение → агент)
- Жёсткое правило: `text` — ТОЛЬКО если просят переработать конкретный текст, не для вопросов
- `calibration_hint` теперь корректно вставляется в промпт

**registry.py:**
- Описание `text` агента уточнено: "трансформирует готовый текст... НЕ для вопросов, разговоров, фактических запросов"
- Ключевое слово `"объясни"` убрано из `explain` (слишком широкое), заменено на `"объясни подробно"`

---

### 2026-04-22 — CalendarAgent: подключение календаря к чату

**Проблема:** `calendar_service.py` и таблица `events` в БД существовали, но ни один агент их не использовал. Добавить или посмотреть события через чат было невозможно.

**Новый файл: `app/agents/calendar_agent.py`**

Умеет:
- Добавить событие: дата, время начала/конца, описание, цвет
- Показать события на день (сегодня/завтра/конкретная дата)
- Показать ближайшие события
- Удалить событие по ID
- Обновить время или название

Разбор дат без LLM: "сегодня", "завтра", "послезавтра", "в пятницу", "15 мая", "15.05", ISO формат.
Операции через XML-теги в ответе LLM → `calendar_service` → SQLite.
Быстрый показ (без LLM) при фразах "покажи события", "что сегодня", "мой календарь".

**Изменены файлы:**

**`app/agents/registry.py`** — добавлен `CalendarAgent` с ключевыми словами: "добавь событие", "запланируй", "встреча в", "расписание на", "созвон", "митинг" и др.

**`app/agents/orchestrator.py`** — добавлен `case "calendar"` в фабрику агентов

**`app/agents/raya_agent.py`** — добавлен `_build_events_block()`: ближайшие события инжектируются в системный промпт Раи, чтобы она знала о расписании даже без явного запроса

**`app/agents/morning_agent.py`** — дайджест теперь включает блок **📅 Календарь** с событиями на сегодня и завтра (параллельный запрос через `asyncio.gather`)
