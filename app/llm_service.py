...
- вопрос про ТЕКУЩЕЕ время/дату/день недели ("сколько времени", "который час",
  "какое сегодня число") — это уже известно системе без поиска, НИКОГДА не искать

ПРАВИЛО: при сомнении — ИСКАТЬ. Лучше лишний поиск чем устаревший ответ.
Исключение — вопросы о текущем времени/дате: там поиск всегда не нужен.
...
- "сколько времени сейчас" → {{"needs_search": false, "query": ""}}
- "который час" → {{"needs_search": false, "query": ""}}\
"""
...
# Вопросы про ТЕКУЩЕЕ время/дату — поиск НИКОГДА не запускаем.
# Точное время уже считается детерминированно в _build_date_block()
# (app/agents/raya_agent.py) и передаётся модели в system-промпте.
# Поиск здесь не просто бесполезен, а вреден: страницы про "текущее
# время" — это статичные снепшоты (часто для другого часового пояса),
# а модели явно велено доверять результатам поиска как "свежим данным"
# (см. _build_hard_rules в raya_agent.py) — из-за этого она подставляла
# случайное время из поисковой выдачи вместо корректно вычисленного.
_TIME_NOW_RE = _re.compile(
    r"(сколько\s+времени|который\s+час|время\s+сейчас|"
    r"текущ(ее|ая|ий)\s+(время|дата|день)|"
    r"какая?\s+(сегодня|сейчас)\s+дата|какое\s+(сегодня|сейчас)\s+число|"
    r"какой\s+(сегодня|сейчас)\s+день"
    r")|"
    r"^(а|и)?\s*(сейчас\s+сколько|сколько\s+сейчас)[\s\?\.\!…]*$",
    _re.IGNORECASE,
)
...
class LLMService:
    ...
    async def _decide_search(self, message: str, user_id: int = 0) -> tuple[bool, str]:
        ...
        # Вопросы про текущее время/дату — никогда не ищем (см. комментарий у _TIME_NOW_RE).
        # Реальное время и так уходит модели в system-промпте, поиск только вредит.
        if _TIME_NOW_RE.search(message.strip()):
            logger.debug("search: вопрос о времени/дате — поиск пропущен")
            return False, ""
