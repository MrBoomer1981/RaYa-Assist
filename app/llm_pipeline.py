...
- "Выбрал конкретную модель для проекта" → {{"decisions": {{"llm_модель": "выбранная-модель"}}}}
...
_RESUME_PROMPT = """\
Пользователь вернулся после паузы {hours:.0f} ч. Вот что обсуждали:
Тема: {topic}
Цель: {user_goal}
Незавершённые темы: {threads}
Краткое резюме: {summary}

Напиши одну короткую фразу-мостик (1 предложение) — напомни пользователю о чём говорили,
чтобы легко продолжить. Тон живой, не формальный.
Обращайся к пользователю по имени. Только фраза, без лишних слов."""
...
class ContextService:
    ...
    async def update(self, user_id: int) -> None:
        ...
        try:
            ...
            history_text = "\n".join(
                f"{'Пользователь' if m.__class__.__name__ == 'HumanMessage' else 'RaYa'}: {m.content[:200]}"
                for m in messages
            )
...
class RouterCalibration:
    ...
    def check_mismatch(self, user_id: int, next_message: str) -> bool:
        """
        Проверяем: следующее сообщение пользователя — сигнал недовольства?
