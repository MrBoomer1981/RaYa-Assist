...
_MSK_OFFSET_HOURS = 3  # МСК = UTC+3, без перехода на летнее/зимнее время с 2014г.


def now_msk() -> datetime:
    """
    Текущее время по Москве — ЦЕЛЫМ timedelta, а не прибавлением к .hour.

    Раньше в нескольких местах дату для "сегодня" считали как
    utcnow().date()/.strftime(...) напрямую, без поправки на МСК. В окне
    UTC 21:00-23:59 (когда в МСК уже следующие сутки, 00:00-02:59) это
    приводило к тому, что события/задачи/дневниковые записи "сегодня"
    молча уходили под ВЧЕРАШНЕЙ датой. Прибавление смещения как timedelta
    (а не только к .hour с последующим % 24) сдвигает дату вместе с часом
    корректно.
    Используй это для всего, что похоже на "какой сегодня день у
    пользователя" — календарь, задачи, дневник, дайджест. Для чистой
    арифметики длительности (сколько часов прошло с X) часовой пояс не
    важен — там можно оставаться на utcnow().
    """
    return utcnow() + timedelta(hours=_MSK_OFFSET_HOURS)


def today_msk_str() -> str:
    """'today' по МСК в формате ГГГГ-ММ-ДД — то, что чаще всего реально нужно."""
