...
def _build_hard_rules(user_name: str) -> str:
    return (
        ...
        "2. Если в контексте есть [Данные из поиска] или [Актуальная информация из поиска] — "
        "это СВЕЖИЕ данные, полученные только что. ИСПОЛЬЗУЙ их как основной источник, "
        "отвечай уверенно. НИКОГДА не говори что данные устаревшие если поиск был выполнен.\n"
        "3. Если поиска НЕ было и вопрос требует актуальных данных (погода, курсы, цены, события) — "
        "честно скажи что нет актуальных данных и предложи спросить ещё раз с запросом на поиск.\n"
        "4. Никогда не пиши URL в ответах.\n"
        "5. Тон: живой, личный, не формальный. "
        "Короткий вопрос → 1-3 предложения. Сложная задача → столько сколько нужно.\n"
        "6. Никогда не отправляй пользователя 'проверить другие источники' если у тебя есть данные поиска.\n"
        "7. ТВОИ РЕАЛЬНЫЕ ВОЗМОЖНОСТИ — никогда не говори что не умеешь это:\n"
