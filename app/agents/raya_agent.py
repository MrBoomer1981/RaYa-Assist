...
def _build_hard_rules(user_name: str) -> str:
    return (
        ...
        "2. Если есть [Данные из поиска] или [Актуальная информация из поиска] — "
        "это ТОЛЬКО ЧТО полученные данные. ИСПОЛЬЗУЙ их и отвечай уверенно. "
        "ЗАПРЕЩЕНО говорить что данные устаревшие если поиск был выполнен.\n"
        "3. Если поиска НЕ было, а вопрос про погоду/курсы/события — скажи что нет актуальных данных, "
        "и предложи спросить снова. НЕ отправляй пользователя на другие сайты.\n"
        "4. Информацию из поиска излагай своими словами. Никаких URL.\n"
