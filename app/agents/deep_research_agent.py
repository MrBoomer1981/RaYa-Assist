...
class DeepResearchAgent(BaseAgent):
    ...
    # Было 480 (8 мин). С правильным backoff в groq_rotator.py (ждём
    # подсказанное Groq время вместо мгновенного провала) отдельные чанки
    # теперь могут реально ждать по несколько секунд при рейт-лимите —
    # это дороже по времени, но доводит дело до результата. Даём запас
    # сверх заявленных ~7 минут для study-режима, а не режем то, что
    # раньше просто тихо падало пустым таймаутом.
